...
public class Game {
	 ...
	 * */
    private static ArrayList<Card> createStack(){//metoda zwracaj¹ca listê z kartami
        ArrayList<Card> deck = new ArrayList<>();

        for(int k = 0; k < 2; k++) {
            for (int i = k; i <= 12; i++) {//najpierw dodajemy taliê od 0 do 12 a póniej od 1 do 12
	...
	// pobranie karty ze stosu zaczynaj¹c od zera a jak pobierzemy o j¹ usuwamy ze stosu
    ...
    public Game(BufferedReader[] inP, PrintWriter[] outP, BufferedReader[] inPG, PrintWriter[] outPG, BufferedReader[] inPR, PrintWriter[] outPR) throws Exception {
    	...
    	// utworzenie talii do gry
    	stack = createStack();
        players = new ArrayList<>();
        Collections.shuffle(stack);

        printInitMessage();//metoda powitalna
    ...
    public void play(){
    	...
    	try {
	        ...
	        while(true) {
	        	...
	        	// dobranie kart
	            while(cardsToDraw --> 0){           //operator d¹¿¹cy do 0
